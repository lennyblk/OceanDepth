...
void use_inventory_item(Player *player)
{
    ...
    case ITEM_HEALTH_KIT:
        if (player->hp < player->max_hp)
        {
            ...
            player->hp = (player->hp + item->hp_restore > player->max_hp) ? player->max_hp : player->hp + item->hp_restore;
    ...
    case ITEM_OXYGEN_CAPSULE:
        if (player->oxygen < player->max_oxygen)
        {
            ...
            player->oxygen = (player->oxygen + item->oxygen_restore > player->max_oxygen) ? player->max_oxygen : player->oxygen + item->oxygen_restore;
